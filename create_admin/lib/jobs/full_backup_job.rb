...
class ::Jobs::FullBackupJob
    ...
    begin
      ...
      data_services.each { | name, attributes |
        ...
        if dir['backup'] == true || dir['backup'] == 'true'
          ...
          configurer = VMC::KNIFE::RecipesConfigurationApplier.new(manifest, client, nil, nil, /^#{name}$/, {:file_names=> full_path_to_backup, :app_name=> @admin_instance.app_name(:intalio), :data_only=>true})
  ...
  def call_extensible_backup(client, filepath)
    app_name_intalio = @admin_instance.app_name(:intalio)
