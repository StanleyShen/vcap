module CloudFoundryAttributes
  ...
  def compute_derived_attributes
    ...
    Chef::Log.info("Compute the 'effective' deployment attributes from the ruby recipe")
    ...
    node[:deployment][:local_run_profile] = File.expand_path(node[:cloudfoundry][:user_home], ".cloudfoundry_deployment_local") unless node[:deployment][:local_run_profile]
