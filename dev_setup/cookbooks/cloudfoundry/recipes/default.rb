...
raise "deployment_name: #{node[:deployment][:name]}; deployment_home: #{node[:deployment][:home]} Not the expected deployment_name ruby_path: #{node[:ruby][:path]}" if node[:ruby][:path] != "/home/ubuntu/cloudfoundry/.deployments/intalio_devbox/deploy/rubies/ruby-1.9.2-p320"
